...
# Thay thế các giá trị NaN trong cột "Vị trí" bằng chuỗi rỗng ''
new_data["Vị trí"] = new_data["Vị trí"].fillna('')
...
def find_similar_chungCu(target_row):
    # Thay thế các giá trị NaN trong target_row
    target_row["Vitri"] = target_row["Vitri"].fillna('')
    ...
    filtered_data["similarity"] = filtered_data.apply(lambda row: calculate_similarity(target_row.iloc[0], row), axis=1)
...
#
# # Tìm các căn nhà tương tự
# similar_houses = find_similar_chungCu(target_row)
#
# # Hiển thị kết quả
